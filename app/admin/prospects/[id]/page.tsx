  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="flex items-start justify-between mb-6">
        <div>
          <Link href="/admin/prospects" className="text-xs text-gray-400 hover:text-gray-700 mb-2 block">← All prospects</Link>
          <h1 className="text-2xl font-semibold text-gray-900">{p.company_name}</h1>
        ...
        <div className="flex flex-col items-end gap-2">
          <span className="text-2xl font-semibold text-gray-900">{p.fit_score ?? '?'}<span className="text-sm text-gray-400">/100</span></span>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-lg p-4 border border-gray-100">
            ...
            <dl className="space-y-2 text-sm">
              <div className="flex justify-between">
                <dt className="text-gray-400">Business type</dt>
                <dd className="text-gray-700">{p.business_type?.replace(/_/g, ' ') ?? '—'}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-400">ABN</dt>
                <dd className="text-gray-700">{p.abn ?? '—'}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-400">Source</dt>
                <dd className="text-gray-700">{p.source ?? '—'}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-400">Postal address</dt>
                <dd className="text-gray-700 text-right max-w-40">{p.postal_address ?? '—'}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-400">Service suburbs</dt>
                <dd className="text-gray-700 text-right max-w-40">{(p.service_suburbs ?? []).join(', ') || '—'}</dd>
              </div>
            </dl>
          </div>
          ...
          {p.ai_summary && (
            <div className="bg-white rounded-lg p-4 border border-gray-100">
              ...
              <p className="text-sm text-gray-500 leading-relaxed">{p.ai_summary}</p>
            </div>
          )}

          {p.fit_reasons?.length > 0 && (
            <div className="bg-white rounded-lg p-4 border border-gray-100">
              <h3 className="text-sm font-medium text-gray-700 mb-2">Fit reasons</h3>
          ...
          {p.demo_slug && (
            <div className="bg-white rounded-lg p-4 border border-gray-100">
              <h3 className="text-sm font-medium text-gray-700 mb-2">Demo page</h3>
              <a
          ...
          {p.notes && (
            <div className="bg-white rounded-lg p-4 border border-gray-100">
              ...
              <p className="text-sm text-gray-500 leading-relaxed whitespace-pre-wrap">{p.notes}</p>
            </div>
          )}
        </div>
        ...
        <div>
          {timeline.length > 0 && (
            <div className="bg-white rounded-lg p-4 border border-gray-100 mb-4">
              <h3 className="text-sm font-medium text-gray-700 mb-3">Contact timeline</h3>
              <div className="space-y-3">
                {timeline.map((e) => (
                  <div key={e.field} className="flex justify-between items-center">
                    <span className="text-xs text-gray-500">{e.label}</span>
                    <span className="text-xs text-gray-400">{new Date(e.value).toLocaleDateString('en-AU')}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto">
                {events.map(ev => (
                  <div key={ev.id} className="flex gap-3">
                    <span className="text-xs text-gray-600 shrink-0">{new Date(ev.occurred_at).toLocaleDateString('en-AU')}</span>
                    <span className="text-xs text-gray-500">{ev.event_type.replace(/_/g, ' ')}</span>
                    {ev.channel && <span className="text-xs text-gray-600">·{ev.channel}</span>}
                  </div>
                ))}
              </div>
            </div>
