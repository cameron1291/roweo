...
export default async function AdminPage() {
  ...
  const [
    ...
    supabase.from('profiles').select('id, plan').eq('subscription_status', 'active'),
  ...
  const PLAN_PRICE: Record<string, number> = { starter: 149, professional: 199, growth: 249 }
  const activeProfiles = activeResult.data ?? []
  const mrrAud = activeProfiles.reduce((sum, p) => sum + (PLAN_PRICE[p.plan as string] ?? 199), 0)
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">{new Date().toLocaleDateString('en-AU', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
        </div>
        <form method="GET">
          <button className="text-xs text-gray-400 hover:text-gray-900 border border-gray-200 rounded px-3 py-1.5 transition-colors">
            Refresh
          </button>
        </form>
      </div>
      ...
      <section className="mb-8">
        <h2 className="text-xs font-medium text-gray-400 uppercase tracking-widest mb-3">Today&apos;s Operations</h2>
      ...
      <section className="mb-8">
        <h2 className="text-xs font-medium text-gray-400 uppercase tracking-widest mb-3">Revenue</h2>
        <div className="grid grid-cols-3 gap-4">
          <StatCard label="MRR (AUD)" value={`$${mrrAud.toLocaleString()}`} sub={`${activeProfiles.length} active subscriber${activeProfiles.length !== 1 ? 's' : ''}`} />
          <StatCard label="Active builders" value={activeProfiles.length} />
      ...
      <div className="grid grid-cols-2 gap-8">
        <section>
          <h2 className="text-xs font-medium text-gray-400 uppercase tracking-widest mb-3">Recent Scraper Runs</h2>
          <div className="bg-white/3 border border-gray-100 rounded-lg divide-y divide-white/5">
            {(recentScraperResult.data ?? []).length === 0 && (
              <p className="text-sm text-gray-400 p-4">No scraper runs yet.</p>
            )}
            {(recentScraperResult.data ?? []).map((run: any) => (
              <div key={run.started_at} className="flex items-center justify-between px-4 py-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-400">{new Date(run.started_at).toLocaleString('en-AU')}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section>
          <h2 className="text-xs font-medium text-gray-400 uppercase tracking-widest mb-3">Latest DAs Ingested</h2>
          <div className="bg-white/3 border border-gray-100 rounded-lg divide-y divide-white/5 overflow-hidden">
            {(recentDasResult.data ?? []).length === 0 && (
              <p className="text-sm text-gray-400 p-4">No DAs yet.</p>
            )}
            {(recentDasResult.data ?? []).map((da: any, i: number) => (
              <div key={i} className="flex items-center justify-between px-4 py-2.5">
                <div>
                  <p className="text-sm">{da.suburb}, {da.state}</p>
                  <p className="text-xs text-gray-400">{da.project_type?.replace(/_/g, ' ')}</p>
                </div>
                <p className="text-xs text-gray-600">{da.lodged_date}</p>
              </div>
            ))}
          </div>
        </section>
